...
if($_POST['key'] == 'addsupplier'){
	...
	$dob=$_POST['dob'];
	if($mobile == '' || $name == '' || $email == '' || $company == '' || $address == '' 
		|| $dob == ''){
		echo json_encode(['key'=>0,'msg'=>'Field Must not be empty']);
	}else{
        
		$sql="INSERT INTO supplier(`mobile`,`name`,`email`,`company`,`address`,`dob`)
		VALUES('$mobile','$name','$email','$company','$address','$dob')";
		$insert=$db->insert($sql);
		if($insert){
			echo json_encode(['key'=>1,'msg'=>'Supplier Added successfully']);
		}else{
